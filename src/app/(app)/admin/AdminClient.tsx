...
export default function AdminClient({ profile, community, activityLog, pendingProfiles, announcements, allCommunities }: Props) {
  ...
  async function sendAnnouncement() {
    ...
    const { error } = await supabase.from('admin_announcements').insert({
      ...
      title: warnTitle,
      body: warnBody || null,
      meeting_date: warnType === 'convocatoria' && meetingDate ? meetingDate : null,
      meeting_location: warnType === 'convocatoria' ? meetingLocation : null,
      meeting_type: warnType === 'convocatoria' ? meetingType : null,
      active: true,
    })
    setSendingWarn(false)
    if (!error) {
      setWarnSuccess('Sent to all residents via in-app + email.')
      setWarnTitle('')
      setWarnBody('')
      setMeetingDate('')
      setMeetingLocation('')
      setTimeout(() => setWarnSuccess(''), 4000)
    }
  }

  async function archiveAnnouncement(id: string) {
    await supabase.from('admin_announcements').update({ active: false }).eq('id', id)
  }

  async function saveConfig() {
    setConfigSaving(true)
    await supabase.from('communities').update({
      rental_min_months: rentalMonths,
      short_term_rental_allowed: shortTermAllowed,
      community_voice_enabled: cvEnabled,
      birthday_wishes_enabled: birthdayEnabled,
      languages_extended: extLangs,
    }).eq('id', profile.community_id)
    setConfigSaving(false)
  }

  function toggleExtLang(lang: string) {
    setExtLangs(prev => prev.includes(lang) ? prev.filter(l => l !== lang) : [...prev, lang])
  }
  ...
  async function uploadDocument() {
    ...
    if (storageError) {
      setDocError(`Upload failed: ${storageError.message}`)
    ...
    // 2. Insert document row (or find existing one with same title)
    ...
    if (existing) {
      ...
      const { data: newDoc, error: docError } = await supabase
        ...
        .single()
      if (docError || !newDoc) {
        setDocError(`Could not create document: ${docError?.message}`)
        setDocUploading(false)
        return
      }
    ...
    if (fileError) {
      setDocError(`File record failed: ${fileError.message}`)
      setDocUploading(false)
      return
    }

    setDocSuccess(`"${docTitle}" uploaded in ${docLang} ✓`)
    setDocTitle('')
    setDocFile(null)
    setDocsLoaded(false) // force reload
    await loadDocs()
    setDocUploading(false)
  }
  ...
  const annBg: Record<string, string> = {
    warning:      'background: #fef2f2; border-color: #fecaca;',
    announcement: 'background: #fffbeb; border-color: #fde68a;',
    convocatoria: 'background: #f0fdf4; border-color: #bbf7d0;',
  }
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto' }}>
      ...
      {activeTab === 'activity' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {activityLog.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--txl)', fontSize: '13px' }}>No activity yet.</div>
          )}
          {activityLog.map(entry => (
            <div key={entry.id} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px' }}>
              <div style={{
                ...
                fontSize: '12px', background: '#dbeafe',
              }}>📋</div>
              <div style={{ flex: 1 }}>
      ...
      {activeTab === 'pending' && (
        <div>
          {pendingProfiles.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--txl)', fontSize: '13px' }}>No pending registrations. 🎉</div>
          )}
          <div className="warn-bar" style={{ marginBottom: '12px' }}>
            These registrations require liaison verification before full access is granted.
          </div>
              ...
              <div key={p.id} className="card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '13px', fontWeight: 600, color: '#fff', background: 'var(--pine)',
                  }}>
                    {p.apt_number}
                  </div>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--txm)' }}>
                      {p.username || 'No username'} · Registered {formatDate(p.created_at)}
                    </div>
                  </div>
                </div>
                {p.occupants && p.occupants.length > 0 && (
                  <div style={{ marginBottom: '12px', fontSize: '11px', color: 'var(--txm)' }}>
                    <span style={{ fontWeight: 500 }}>Occupants:</span>{' '}
                    {p.occupants.map((o: any) => o.name || 'Unnamed').join(', ')}
                  </div>
                )}
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => approveProfile(p.id)} className="btn btn-primary btn-sm">✓ Approve</button>
                  <button onClick={() => rejectProfile(p.id)} className="btn btn-danger btn-sm">Reject</button>
        ...
        <div className="two-col" style={{ gap: '16px' }}>
          <div>
            <div style={{ fontSize: '11px', color: 'var(--txm)', marginBottom: '12px' }}>
              Post to all residents — displayed on Dashboard and sent via in-app + email.
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
              {warnTypeBtns.map(type => (
                <button
                  key={type.id}
                  onClick={() => setWarnType(type.id)}
                  style={{
                    flex: 1, padding: '8px', borderRadius: '12px', fontSize: '11px', fontWeight: 500,
                    border: warnType === type.id ? '1px solid transparent' : '1px solid var(--br)',
                    background: warnType === type.id ? type.activeBg : 'transparent',
                    color: warnType === type.id ? type.activeColor : 'var(--txm)',
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                <label className="form-label">
                  {warnType === 'convocatoria' ? 'Meeting title' : 'Message / title'} *
                </label>
                <input
                  required
                  value={warnTitle}
                  ...
                  placeholder={
                    warnType === 'warning' ? 'e.g. Water cut Tuesday 10 Jun, 10:00–14:00' :
                    warnType === 'announcement' ? 'e.g. Pool opens for the season this Saturday' :
                    'e.g. Ordinary general meeting — June 2026'
                  }
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Additional information for residents..."
                  rows={3}
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    <div>
                      ...
                      <input type="datetime-local" value={meetingDate} onChange={e => setMeetingDate(e.target.value)} className="form-input" />
                    </div>
                    <div>
                      <label className="form-label">Location</label>
                      <input value={meetingLocation} onChange={e => setMeetingLocation(e.target.value)} placeholder="Party room / Zoom" className="form-input" />
                    </div>
                  </div>
                  <div>
                    <label className="form-label">Meeting type</label>
                    <select value={meetingType} onChange={e => setMeetingType(e.target.value as any)} className="form-select">
                      <option value="ordinary">Ordinary (Junta ordinaria)</option>
                      <option value="extraordinary">Extraordinary (Junta extraordinaria)</option>
                    </select>
                  </div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--tx)' }}>Auto-generate PDF for residents to download</span>
                  </label>
                </>
              )}

              {warnSuccess && (
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '8px', fontSize: '11px', color: '#166534' }}>
                  ✓ {warnSuccess}
                </div>
              )}
              ...
              <button
                ...
                style={{ width: '100%', opacity: sendingWarn ? 0.6 : 1 }}
              >
                {sendingWarn ? 'Sending...' : '📨 Send to all residents'}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {announcements.length === 0 && (
                <div style={{ fontSize: '11px', color: 'var(--txl)', textAlign: 'center', padding: '16px 0' }}>No announcements posted yet.</div>
              )}
              {announcements.map(ann => (
                <div key={ann.id} style={{
                  borderRadius: '12px', border: '1px solid',
                  padding: '12px', display: 'flex', alignItems: 'flex-start', gap: '8px',
                  background: ann.type === 'warning' ? '#fef2f2' : ann.type === 'announcement' ? '#fffbeb' : '#f0fdf4',
                  borderColor: ann.type === 'warning' ? '#fecaca' : ann.type === 'announcement' ? '#fde68a' : '#bbf7d0',
                }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '9px', color: 'var(--txl)', marginTop: '4px' }}>
                      {formatDate(ann.created_at)} · {ann.active ? 'Active' : 'Archived'}
                    </div>
                  </div>
                  {ann.active && (
                    <button
      ...
      {activeTab === 'documents' && (
        <div className="two-col" style={{ gap: '24px', alignItems: 'flex-start' }}>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Acta Junta General Ordinària 2025"
                  className="form-input"
                />
                <div style={{ fontSize: '10px', color: 'var(--txl)', marginTop: '4px' }}>
                  If a document with this title already exists, the file will be added as a new language version.
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  style={{ fontSize: '12px', color: 'var(--tx)', width: '100%' }}
                />
                {docFile && (
              ...
              {docError && (
                <div style={{ fontSize: '11px', color: '#dc2626', background: '#fef2f2', padding: '8px 12px', borderRadius: '8px' }}>
                  {docError}
                </div>
              )}
              {docSuccess && (
                <div style={{ fontSize: '11px', color: '#166534', background: '#f0fdf4', padding: '8px 12px', borderRadius: '8px' }}>
                  ✓ {docSuccess}
                </div>
              )}
              <button
                ...
                className="btn btn-primary"
                style={{ width: '100%', opacity: docUploading || !docFile || !docTitle.trim() ? 0.6 : 1 }}
              >
          ...
          <div>
            ...
            {docs.length === 0 && (
              <div style={{ fontSize: '12px', color: 'var(--txl)', textAlign: 'center', padding: '32px 0' }}>
                No documents yet.
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {docs.map(doc => (
                <div key={doc.id} className="card" style={{ padding: '12px' }}>
                  ...
                  <div style={{ fontSize: '10px', color: 'var(--txl)', marginBottom: '8px' }}>
                    {DOC_CATEGORIES.find(c => c.id === doc.category)?.label || doc.category} · {formatDate(doc.created_at)}
                  </div>
                  {/* Language files */}
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    ...
                    {(doc.files || []).length === 0 && (
                      <span style={{ fontSize: '10px', color: 'var(--txl)', fontStyle: 'italic' }}>No files yet</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '20px' }}>
              {CORE_LANGS.map(lang => (
                <div key={lang} style={{
                  ...
                  fontSize: '12px', fontWeight: 600, color: '#fff', background: 'var(--pine)',
                }}>
                  {lang}
                </div>
              ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              {ALL_EXT_LANGS.map(lang => (
                <button
                  key={lang}
                  onClick={() => toggleExtLang(lang)}
                  style={{
                    padding: '8px', borderRadius: '12px', fontSize: '12px', fontWeight: 600,
                    cursor: 'pointer', transition: 'all 0.15s',
                    background: extLangs.includes(lang) ? '#dcfce7' : 'transparent',
                    color: extLangs.includes(lang) ? '#166534' : 'var(--txm)',
                    border: extLangs.includes(lang) ? '1px solid transparent' : '1px solid var(--br)',
                  }}
                >
                  {lang}
                </button>
              ...
              <button style={{
                padding: '8px', borderRadius: '12px', fontSize: '12px', color: 'var(--txl)',
                border: '1px dashed var(--br)', background: 'transparent', cursor: 'pointer',
              }}>+ add</button>
            </div>
          </div>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
                ...
                <input
                  ...
                  min="1" max="60"
                  style={{ width: '56px', padding: '4px 8px', border: '1px solid var(--br)', borderRadius: '8px', fontSize: '12px', textAlign: 'center' }}
                />
              </div>
              ...
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--br)', padding: '10px 0' }}>
                  ...
                  <button
                    ...
                    className="toggle"
                    style={{ background: item.value ? 'var(--pine)' : 'var(--br)' }}
                  >
                    <span style={{
              ...
              {/* Banner URL */}
              <div style={{ borderTop: '1px solid var(--br)', paddingTop: '10px' }}>
                <span style={{ fontSize: '12px', color: 'var(--tx)', display: 'block', marginBottom: '4px' }}>Banner image URL</span>
                <input
              ...
              <button
                ...
                className="btn btn-primary"
                style={{ width: '100%', marginTop: '12px', opacity: configSaving ? 0.6 : 1 }}
              >
      ...
      {activeTab === 'tenants' && isSA && (
        <div>
          <div style={{ fontSize: '11px', color: 'var(--txm)', marginBottom: '16px' }}>
            All tenant communities. Each is fully independent. Bermar = pilot #001.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {allCommunities.map(comm => (
              <div key={comm.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{
                  ...
                  background: comm.primary_color || 'var(--pine)',
                }}>
                  {comm.name.slice(0, 2).toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)', fontFamily: 'DM Serif Display, serif' }}>
                    {comm.name}
                  </div>
                  <div style={{ fontSize: '11px', color: 'var(--txm)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {comm.address}
                  </div>
                  <div style={{ fontSize: '9px', color: 'var(--txl)', marginTop: '2px' }}>
                    {comm.languages_core.join(' · ')} · Total apts TBC: {comm.total_apts_tbc}
                  </div>
                </div>
            ...
            {/* Add new community */}
            <button style={{
              width: '100%', borderRadius: '16px', border: '2px dashed var(--br)',
              padding: '16px', display: 'flex', alignItems: 'center', gap: '12px',
              color: 'var(--txl)', background: 'transparent', cursor: 'pointer',
            ...
            >
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', border: '1px solid var(--br)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>+</div>
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: '13px', fontWeight: 500 }}>Add new community</div>
                <div style={{ fontSize: '11px' }}>Clone Bermar config · customise all variables · new tenant</div>
              </div>
            </button>

            {/* SA roadmap note */}
            <div style={{ borderRadius: '12px', padding: '12px', fontSize: '11px', color: 'var(--txm)', lineHeight: 1.6, background: 'var(--sand-d)' }}>
              <div style={{ fontWeight: 500, color: 'var(--tx)', marginBottom: '4px' }}>SA tools roadmap (mid-term)</div>
              Platform metrics across all tenants · Weekly digest per community · Suspend community or user · GDPR data export ·
              Audit log · Progressive feature rollout · Billing management · Marketing landing page for onboarding new communities.
            </div>
          </div>
        </div>
      )}
