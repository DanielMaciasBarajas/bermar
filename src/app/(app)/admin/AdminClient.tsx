...
export default function AdminClient({ profile, community, activityLog, pendingProfiles, announcements, allCommunities }: Props) {
  ...
  async function sendAnnouncement() {
    ...
    if (!error) {
      ...
      setWarnTitle(''); setWarnBody(''); setMeetingDate(''); setMeetingLocation('')
  ...
  async function uploadDocument() {
    ...
    if (existing) {
      ...
      const { data: newDoc, error: docErr } = await supabase.from('documents').insert({
        community_id: profile.community_id, title: docTitle.trim(), category: docCategory,
      }).select().single()
            ...
            <div key={entry.id} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {entry.metadata && Object.keys(entry.metadata).length > 0 && (
                  <div style={{ fontSize: '11px', color: 'var(--txm)', marginTop: '2px' }}>{JSON.stringify(entry.metadata).slice(0, 80)}</div>
                )}
              </div>
              ...
              <div key={p.id} className="card">
                ...
                {p.occupants && p.occupants.length > 0 && (
                  <div style={{ marginBottom: '12px', fontSize: '11px', color: 'var(--txm)' }}>
                    <span style={{ fontWeight: 500 }}>Occupants:</span> {p.occupants.map((o: any) => o.name || 'Unnamed').join(', ')}
                  </div>
                )}
      ...
      {activeTab === 'warnings' && (
        <div className="two-col" style={{ gap: '16px' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
              {warnTypeBtns.map(type => (
                <button key={type.id} onClick={() => setWarnType(type.id)} style={{
                  flex: 1, padding: '8px', borderRadius: '12px', fontSize: '11px', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s',
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                ...
                <input required value={warnTitle} onChange={e => setWarnTitle(e.target.value)}
                  placeholder={warnType === 'warning' ? 'e.g. Water cut Tuesday 10 Jun, 10:00–14:00' : warnType === 'announcement' ? 'e.g. Pool opens for the season this Saturday' : 'e.g. Ordinary general meeting — June 2026'}
                  className="form-input" />
              </div>
              <div>
                <label className="form-label">Details (optional)</label>
                <textarea value={warnBody} onChange={e => setWarnBody(e.target.value)} placeholder="Additional information for residents..." rows={3} className="form-textarea" />
              </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {announcements.length === 0 && <div style={{ fontSize: '11px', color: 'var(--txl)', textAlign: 'center', padding: '16px 0' }}>No announcements posted yet.</div>}
          ...
          <div>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button onClick={uploadDocument} disabled={docUploading || !docFile || !docTitle.trim()} className="btn btn-primary" style={{ width: '100%', opacity: docUploading || !docFile || !docTitle.trim() ? 0.6 : 1 }}>
                {docUploading ? tc('loading') : '⬆ Upload document'}
              </button>
            </div>
          </div>
          <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {(doc.files || []).map((f: any) => (
                      <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        <button onClick={() => deleteDocFile(f.id)} style={{ fontSize: '10px', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', padding: '0 2px' }} title="Remove this file">✕</button>
                      </div>
                    ))}
        ...
        <div className="two-col" style={{ gap: '24px' }}>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '20px' }}>
              {CORE_LANGS.map(lang => (
                <div key={lang} style={{ padding: '8px', borderRadius: '12px', textAlign: 'center', fontSize: '12px', fontWeight: 600, color: '#fff', background: 'var(--pine)' }}>{lang}</div>
              ))}
            </div>
            <div className="section-title" style={{ marginBottom: '12px' }}>Extended — toggle per community</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              {ALL_EXT_LANGS.map(lang => (
                <button key={lang} onClick={() => toggleExtLang(lang)} style={{
                  padding: '8px', borderRadius: '12px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s',
          ...
          <div>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
              ...
              <div style={{ borderTop: '1px solid var(--br)', paddingTop: '10px' }}>
                ...
                <input defaultValue={community?.banner_url || ''} placeholder="https://..." className="form-input" />
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {allCommunities.map(comm => (
              <div key={comm.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '12px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '14px', background: comm.primary_color || 'var(--pine)' }}>
                  {comm.name.slice(0, 2).toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)', fontFamily: 'DM Serif Display, serif' }}>{comm.name}</div>
                  <div style={{ fontSize: '11px', color: 'var(--txm)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{comm.address}</div>
                  <div style={{ fontSize: '9px', color: 'var(--txl)', marginTop: '2px' }}>{comm.languages_core.join(' · ')} · Total apts TBC: {comm.total_apts_tbc}</div>
                </div>
            ...
            <button style={{ width: '100%', borderRadius: '16px', border: '2px dashed var(--br)', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--txl)', background: 'transparent', cursor: 'pointer' }}>
