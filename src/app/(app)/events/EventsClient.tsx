...
export default function EventsClient({ upcoming, live, past, voicePosts, profile }: Props) {
  ...
  async function joinBooking(bookingId: string) {
    ...
    await supabase.from('booking_participants').upsert({
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {upcoming.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--txl)', fontSize: '13px' }}>
              No upcoming open events. Book a premise and open it to neighbours!
            </div>
          )}
            ...
            return (
              <div key={booking.id} className="card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)' }}>
                    {booking.premise?.name} · @{booking.apt_number}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
                  <div style={{ ...avatarBase, background: 'var(--pine)' }}>
                    {booking.apt_number.slice(0, 2)}
                  </div>
                  {booking.participants.map(p => (
                    <div key={p.profile_id} style={{ ...avatarBase, background: '#4b5563' }}>
                      {p.apt_number.slice(0, 2)}
                    </div>
                  ))}
                  {spotsLeft > 0 && Array.from({ length: spotsLeft }).map((_, i) => (
                    <div key={i} style={{ ...avatarBase, background: '#dbeafe', color: '#1e40af', fontWeight: 500 }}>
                      +1
                    </div>
                  ))}
                  {spotsLeft > 0 && (
                    <span style={{ fontSize: '11px', color: '#2563eb', marginLeft: '4px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {!isParticipant && spotsLeft > 0 && (
                      <button
                        ...
                        style={{ opacity: joiningId === booking.id ? 0.6 : 1 }}
                      >
                        {joiningId === booking.id ? 'Joining...' : 'Join'}
                      </button>
                    )}
                    {isParticipant && (
                      <button onClick={() => leaveBooking(booking.id)} className="btn btn-sm">
                        Leave
                      </button>
                    )}
                    {spotsLeft === 0 && !isParticipant && (
                      <span style={{ fontSize: '11px', color: 'var(--txl)', padding: '6px 0' }}>Full</span>
                    )}
                  </div>
                )}
              </div>
            ...
            return (
              <div key={booking.id} className="card">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
                  ...
                  {booking.invite_open && spotsLeft > 0 && Array.from({ length: Math.min(spotsLeft, 3) }).map((_, i) => (
                    <div key={i} style={{ ...avatarBase, background: '#dbeafe', color: '#1e40af' }}>+</div>
                  ))}
                </div>

                {/* Photo + join */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingTop: '10px', borderTop: '1px solid var(--br)' }}>
                  <button style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: 'var(--txl)', background: 'none', border: 'none', cursor: 'pointer' }}>
                    📷 Add photo (optional · visible to all)
                  </button>
      ...
      {activeTab === 'past' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          {past.map(booking => {
            ...
            const voiceText = voicePost
              ? (typeof voicePost.body === 'object'
                  ? voicePost.body.EN || voicePost.body.CA || Object.values(voicePost.body)[0]
                  : voicePost.body)
              : null
            ...
            return (
              <div key={booking.id} className="cv-card" style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '14px', background: 'var(--pine)',
                }}>
                  🎾
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--pine)', marginBottom: '4px' }}>
                    {booking.premise?.name} · {formatDate(booking.date)}
                  </div>
                  {voiceText ? (
                    <p style={{ fontSize: '13px', fontStyle: 'italic', lineHeight: 1.4, marginBottom: '8px', fontFamily: 'DM Serif Display, serif' }}>
                      {voiceText}
                    </p>
                  ) : (
                    <p style={{ fontSize: '11px', color: 'var(--txm)', marginBottom: '8px' }}>
                      {booking.participants.length > 0
                        ? `${booking.participants.length + 1} participants`
                        : `Booked by @${booking.apt_number}`}
                      {booking.slot_start && ` · ${formatTime(booking.slot_start)}–${formatTime(booking.slot_end || '')}`}
                    </p>
                  )}
                  <div style={{ display: 'flex', gap: '4px' }}>
                    <div style={{ ...avatarBase, width: '20px', height: '20px', fontSize: '7px', background: 'var(--pine)' }}>
                      {booking.apt_number.slice(0, 2)}
                    </div>
                    {booking.participants.slice(0, 5).map((p, i) => (
                      <div key={i} style={{ ...avatarBase, width: '20px', height: '20px', fontSize: '7px', background: '#6b7280' }}>
                        {p.apt_number.slice(0, 2)}
                      </div>
                    ))}
                  </div>
