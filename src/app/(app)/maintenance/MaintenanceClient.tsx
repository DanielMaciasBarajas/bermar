...
export default function MaintenanceClient({ profile, community, myTickets }: Props) {
  ...
  async function handleSubmit() {
    ...
    setSaving(false)
    if (!error) { setSubmitted(true); setCategory(''); setLocation(''); setDescription(''); setUrgency('normal'); setInsuranceFlag(false) }
  }

  return (
    <div className="two-col" style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div>
        <div className="section-title" style={{ marginBottom: '8px' }}>{t('submit_ticket')}</div>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div>
            ...
            <input value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Pool pump room, 3rd floor corridor..." className="form-input" />
          </div>

          <div>
            <label className="form-label">{t('description')} <span style={{ color: '#ef4444' }}>*</span></label>
            <textarea required value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe clearly: what, where, when it started, how urgent." rows={3} className="form-textarea" />
          </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }}>
          {[
            ...
            <div key={step.n} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              {i < arr.length - 1 && <div style={{ height: '1px', width: '24px', background: 'var(--br)', marginBottom: '16px' }} />}
            </div>
          ))}
        </div>

        <div className="section-title" style={{ marginBottom: '8px' }}>{community?.moha_name || 'Moha'} — maintenance</div>
        <div className="card" style={{ marginBottom: '16px' }}>
          ...
          <div style={{ fontSize: '10px', color: 'var(--txl)', marginTop: '4px' }}>Not on-call. For emergencies outside hours, use admin company emergency line.</div>
        </div>

        <div className="section-title" style={{ marginBottom: '8px' }}>My tickets</div>
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {myTickets.length === 0 && <p style={{ fontSize: '11px', color: 'var(--txl)', textAlign: 'center', padding: '16px' }}>No tickets submitted yet.</p>}
          {myTickets.map((ticket, i) => (
            <div key={ticket.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px', borderTop: i === 0 ? 'none' : '1px solid var(--br)' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '12px', fontWeight: 500, color: 'var(--tx)' }}>{TICKET_CATEGORY_LABELS[ticket.category] || ticket.category}</div>
