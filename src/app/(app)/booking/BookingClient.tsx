...
export default function BookingClient({ premises, existingBookings, profile }: Props) {
  ...
  function getDateStatus(day: number) {
    ...
    if (bookings.some(b => b.premise_id === selectedPremise?.id && b.date === dateStr && b.profile_id === profile.id)) return 'mine'
    if (bookings.some(b => b.premise_id === selectedPremise?.id && b.date === dateStr)) return 'booked'
    return 'free'
  }
  ...
  async function bookSlot(start: string, end: string) {
    ...
    if (!error) {
      setBookings(prev => [...prev, {
        ...
        invite_open: inviteOpen, invite_scope: (inviteOpen ? inviteScope : 'none') as any,
  ...
  const successBlock = success ? (
    <div style={{ marginTop: '12px', background: '#f0fdf4', color: '#166534', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '12px', fontSize: '12px', textAlign: 'center' }}>
      ...
      <button onClick={() => window.location.reload()}
        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 12px', borderRadius: '8px', background: '#fff', border: '1px solid #bbf7d0', color: '#166534', fontSize: '11px', fontWeight: 500, cursor: 'pointer', marginBottom: calendarUrl ? '10px' : 0 }}>
        {t('refresh_notice')}
      </button>
      {calendarUrl && (
        <div>
      ...
      <div className="booking-grid" style={{ display: 'grid', gridTemplateColumns: '200px 1fr 1fr', gap: '16px', alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {premises.map(p => (
              <button key={p.id} onClick={() => { setSelectedPremise(p); setInviteOpen(false) }}
                style={{
                          ...
                          <div key={slot.start} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {status === 'mine' && (
                              <button onClick={() => booking && cancelBooking(booking.id)}
                                style={{ fontSize: '10px', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer' }}>x</button>
                            )}
                          </div>
                        )
                      })}
