        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: '380px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '28px repeat(10, 1fr)', gap: '3px', marginBottom: '4px' }}>
              <div />
              {DOORS.map(d => (
                <div key={d} style={{ textAlign: 'center', fontSize: '9px', fontWeight: 500, color: 'var(--txl)' }}>{d}</div>
              ))}
            </div>
            ...
            {FLOORS.map(floor => (
              <div key={floor} style={{ display: 'grid', gridTemplateColumns: '28px repeat(10, 1fr)', gap: '3px', marginBottom: '3px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: '6px', fontSize: '9px', fontWeight: 500, color: 'var(--txl)' }}>
                  {floor}
                </div>
                {DOORS.map(door => {
                      ...
                      style={{
                        ...
                        border: isMe
                          ...
                          : hasRegistration && !isOptedOut
                            ? '1px solid rgba(26,61,43,0.2)'
                            : '1px solid var(--br)',
                        background: hasRegistration && !isOptedOut
                          ? 'rgba(26,61,43,0.08)'
                          : 'var(--sand-d)',
                    ...
                    >
                      {hasRegistration && !isOptedOut ? (
                        <>
                          <div style={{
                            ...
                            background: 'var(--pine)', color: '#fff', fontWeight: 600,
                            fontSize: '6px', margin: '2px auto 0',
                          }}>
                            {aptNumber.slice(0, 2)}
                          </div>
                          <div style={{ fontSize: '7px', fontWeight: 500, color: 'var(--pine)', marginTop: '2px' }}>
                            {aptNumber}{isDuplex ? '*' : ''}
                          </div>
                        </>
                      ) : isOptedOut ? (
                        <>
                          <div style={{
                            width: '20px', height: '20px', borderRadius: '50%',
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            background: 'var(--sand-d)', color: 'var(--txl)',
                            fontSize: '8px', margin: '2px auto 0',
                          }}>—</div>
                        ...
                        <div style={{
                          width: '20px', height: '20px', borderRadius: '50%',
                          background: 'var(--br)', margin: '2px auto 0',
                        }} />
                      )}
                      ...
                      {hoveredApt === aptNumber && hasRegistration && !isOptedOut && (
                        <div style={{
                          ...
                          background: '#fff', borderRadius: '8px', border: '1px solid var(--br)',
                          boxShadow: '0 4px 12px rgba(0,0,0,0.1)', padding: '8px',
                          minWidth: '112px', textAlign: 'left', fontSize: '10px',
                        }}>
                          <div style={{ fontWeight: 500, color: 'var(--tx)', marginBottom: '4px' }}>
                            Apt {aptNumber}{isDuplex ? ` / 9${door}` : ''}
                          </div>
                          {profile.show_names
                            ? occupantsByProfile.get(profile.id)?.map(o => (
                                <div key={o.profile_id + o.name} style={{ color: 'var(--txm)' }}>{o.name}</div>
                              ))
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px' }}>
            ...
            <div key={contact.id} className="emerg-card">
              <div style={{
                ...
                {contact.name.includes('Moha') ? '🔧' :
                 contact.name.includes('Liaison') ? '👤' :
                 contact.name.includes('Admin') ? '🏢' : '🆘'}
              </div>
              <div style={{ flex: 1 }}>
              ...
              {contact.phone && (
                <a href={`tel:${contact.phone}`} style={{ fontSize: '11px', fontWeight: 500, color: 'var(--pine)', textDecoration: 'none', flexShrink: 0 }}>
                  {contact.phone}
                </a>
              )}
            </div>
          ...
          {emergencyContacts.length === 0 && (
            <div style={{ fontSize: '11px', color: 'var(--txl)', textAlign: 'center', padding: '16px 0' }}>
              Emergency contacts not yet configured. Admin will add them.
            </div>
          )}
        </div>

        {/* Legend card */}
        <div className="section-title" style={{ marginBottom: '8px' }}>Legend</div>
        <div className="card" style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px', color: 'var(--txm)' }}>
            <div>Pine green cells = registered residents</div>
            <div>Empty cells = apartment not yet registered</div>
            <div>Dash (—) = opted out of directory</div>
            <div>8H/9H* = duplex apartment (both doors, one account)</div>
            <div>Hover any cell to see occupant names (if not opted out)</div>
          </div>
        </div>
        ...
        <div className="card">
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <div style={{
              width: '32px', height: '32px', borderRadius: '50%', flexShrink: 0,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '11px', fontWeight: 600, color: '#fff', background: 'var(--pine)',
            }}>
              {currentProfile.apt_number}
            </div>
            <div>
              ...
              <div style={{ fontSize: '11px', color: 'var(--txm)' }}>
                {currentProfile.show_in_directory ? 'Visible in directory' : 'Hidden from directory'}
              </div>
            </div>
          </div>
          <div style={{ fontSize: '11px', color: 'var(--txl)' }}>
            To change your visibility, go to Settings → Privacy.
          </div>
        </div>
      </div>
    </div>
