...
export default function SettingsClient({ profile, apartment, email }: Props) {
  ...
  // Occupants
  const [occupants, setOccupants] = useState<any[]>(profile?.occupants || [])

  // Interests
  const [selectedInterests, setSelectedInterests] = useState<string[]>(
    (profile?.interests || []).map((i: any) => i.interest)
  )

  // Privacy
  const [privacy, setPrivacy] = useState({
    show_names: profile?.show_names ?? true,
    show_ages: profile?.show_ages ?? false,
    show_interests: profile?.show_interests ?? true,
    show_phone: profile?.show_phone ?? false,
    show_in_directory: profile?.show_in_directory ?? true,
    birthday_wishes: profile?.birthday_wishes ?? true,
    email_notifications: profile?.email_notifications ?? true,
    google_calendar_sync: profile?.google_calendar_sync ?? true,
  })
  ...
  async function saveSettings() {
    ...
    // Save interests — delete all and re-insert
    await supabase.from('interests').delete().eq('profile_id', profile.id)
    if (selectedInterests.length > 0) {
      await supabase.from('interests').insert(
        selectedInterests.map(interest => ({ profile_id: profile.id, interest }))
      )
    }
  ...
  async function deleteAccount() {
    ...
    // Profile is gone so the user effectively has no access
    ...
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
        <div style={{
          ...
          fontSize: '18px', fontWeight: 700, color: '#fff', background: 'var(--pine)',
        }}>
          {profile.apt_number}
        </div>
        <div>
          <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--tx)', fontFamily: 'DM Serif Display, serif' }}>
            Apt {profile.apt_number}
            {apartment?.is_duplex ? ` / ${apartment.duplex_upper_number} (duplex)` : ''}
          </div>
          <div style={{ fontSize: '11px', color: 'var(--txm)' }}>{email}</div>
          <div style={{ fontSize: '11px', color: 'var(--txl)' }}>
            {profile.role} · {profile.approved ? 'Verified' : 'Pending verification'}
          </div>
        </div>
      </div>

      {/* Occupants */}
      <div className="section-title" style={{ marginBottom: '8px' }}>Who lives here</div>
      <div className="card" style={{ marginBottom: '16px' }}>
          ...
          {occupants.map((occ, i) => (
            <div key={occ.id || i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '10px', fontWeight: 600, color: '#fff', background: 'var(--pine)',
              }}>
                {occ.name?.slice(0, 1) || '?'}
              </div>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '10px', color: 'var(--txl)' }}>
                  {occ.gender}{occ.age ? ` · ${occ.age}y` : ''}
                  {occ.birthday_day && occ.birthday_month ? ` · 🎂 ${occ.birthday_day}/${occ.birthday_month}` : ''}
                </div>
              </div>
        ...
        <div style={{ fontSize: '11px', color: 'var(--txl)' }}>
          To update occupants, contact your community admin.
        </div>
      </div>

      {/* Interests */}
      <div className="section-title" style={{ marginBottom: '8px' }}>Interests</div>
      <div className="card" style={{ marginBottom: '16px' }}>
      ...
      {/* Privacy */}
      <div className="section-title" style={{ marginBottom: '8px' }}>Privacy & notifications</div>
      <div className="card" style={{ marginBottom: '16px' }}>
        {[
      ...
      {/* Save */}
      {saved && (
        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '12px', marginBottom: '12px', fontSize: '12px', color: '#166534', textAlign: 'center' }}>
          {saved}
        </div>
      )}
      <button
        ...
        style={{ width: '100%', marginBottom: '32px', opacity: saving ? 0.6 : 1 }}
      >
        {saving ? 'Saving...' : 'Save settings'}
      </button>

      {/* Danger zone */}
      <div className="section-title" style={{ marginBottom: '8px', color: '#dc2626' }}>Danger zone</div>
      <div className="card" style={{ border: '1px solid #fecaca' }}>
        <div style={{ fontSize: '13px', fontWeight: 500, color: '#dc2626', marginBottom: '4px' }}>Delete account</div>
        ...
        {!deleteConfirm ? (
          <button
            ...
            style={{
              ...
              background: 'transparent', border: '1px solid #fecaca', color: '#dc2626', cursor: 'pointer',
            }}
          >
            Delete my account
          </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ fontSize: '12px', color: 'var(--tx)' }}>
              Type your apartment number <strong>{profile.apt_number}</strong> to confirm:
            </div>
            <input
              value={deleteInput}
              onChange={e => setDeleteInput(e.target.value.toUpperCase())}
              placeholder={profile.apt_number}
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button
                ...
                disabled={deleteInput !== profile.apt_number || deleting}
                style={{
                  flex: 1, padding: '8px', borderRadius: '10px', fontSize: '12px', fontWeight: 500,
                  background: deleteInput === profile.apt_number ? '#dc2626' : '#fecaca',
                  color: '#fff', border: 'none', cursor: deleteInput === profile.apt_number ? 'pointer' : 'default',
