...
const STATUS_TAG: Record<string, string> = {
  open: 'tag tag-green',
  voting: 'tag tag-amber',
  resolved: 'tag tag-gray',
  promoted: 'tag tag-blue',
}

const CATEGORY_TAG: Record<string, string> = {
  social: 'tag tag-green',
  infrastructure: 'tag tag-blue',
  rules: 'tag tag-pine',
  complaint: 'tag tag-red',
  project: 'tag tag-amber',
  meeting: 'tag tag-gray',
  other: 'tag tag-gray',
}
...
export default function ProposalsClient({ proposals, profile }: { proposals: ProposalData[]; profile: Profile & { preferred_lang?: string } }) {
  ...
  function getBody(p: ProposalData): string {
    if (p.body_translations && typeof p.body_translations === 'object') {
      return p.body_translations[lang]
        || p.body_translations['ES']
        || p.body_translations['CA']
        || p.body_translations['EN']
        || Object.values(p.body_translations)[0] as string
        || p.body
    }
    return p.body
  }
  ...
  async function vote(proposalId: string, vote: 'support' | 'against') {
    ...
    if (existing) {
      ...
      await supabase.from('proposal_votes').upsert({ proposal_id: proposalId, profile_id: profile.id, vote })
    }
  }

  async function setFlag(proposalId: string, field: 'is_important' | 'is_following' | 'is_dismissed', value: boolean) {
    await supabase.from('proposal_flags').upsert({
      proposal_id: proposalId, profile_id: profile.id, [field]: value
    })
  }
    ...
    <div style={{ maxWidth: '720px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' }}>
        {[
          ...
          <button
            key={f.key}
            onClick={() => setPersonalFilter(f.key)}
            style={{
              ...chipBase,
              background: personalFilter === f.key ? 'var(--pine)' : '#fff',
              color: personalFilter === f.key ? '#fff' : 'var(--txm)',
              borderColor: personalFilter === f.key ? 'var(--pine)' : 'var(--br)',
            }}
          >
            {f.label}
          </button>
        ...
        <div className="card" style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <input
              ...
              placeholder="Title — clear and specific"
            ...
            <textarea
              ...
              placeholder="Describe your proposal. Add translations below for other languages (ES: ... · FR: ...)"
              rows={4}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        {filtered.map(p => {
          ...
          const closesIn = p.voting_closes_at
            ? Math.ceil((new Date(p.voting_closes_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
            : null
          ...
          return (
            <div key={p.id} className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
                <span className={CATEGORY_TAG[p.category] || 'tag tag-gray'}>
                  {CATEGORY_LABELS[p.category] || p.category}
                </span>
                <span className={STATUS_TAG[p.status] || 'tag tag-gray'}>
                  {p.status === 'voting' ? 'Voting active' : p.status.charAt(0).toUpperCase() + p.status.slice(1)}
                </span>
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '4px' }}>
                  <button
                    ...
                    style={{
                      ...chipBase,
                      padding: '2px 8px',
                      background: myFlag?.is_important ? '#fee2e2' : 'transparent',
                      color: myFlag?.is_important ? '#b91c1c' : 'var(--txl)',
                      borderColor: myFlag?.is_important ? 'transparent' : 'var(--br)',
                    }}
                  >⭐</button>
                  <button
                    ...
                    style={{
                      ...chipBase,
                      padding: '2px 8px',
                      background: myFlag?.is_following ? '#dbeafe' : 'transparent',
                      color: myFlag?.is_following ? '#1e40af' : 'var(--txl)',
                      borderColor: myFlag?.is_following ? 'transparent' : 'var(--br)',
                    }}
                  >🔖</button>
                  <button
                    onClick={() => setFlag(p.id, 'is_dismissed', !myFlag?.is_dismissed)}
                    style={{ ...chipBase, padding: '2px 8px', background: 'transparent', color: 'var(--txl)', borderColor: 'var(--br)' }}
                  >🙈</button>
                </div>
              </div>
              ...
              <div style={{ fontSize: '11px', color: 'var(--txm)', marginBottom: '6px' }}>
                ...
                {p.tagged_apts.length > 0 && (
                  <span> · tagged <strong>{p.tagged_apts.map(a => `@${a}`).join(' ')}</strong></span>
                )}
                {p.tag_all && <span> · <strong>@tots</strong></span>}
              </div>
              <p style={{ fontSize: '11px', color: 'var(--txm)', lineHeight: 1.5, marginBottom: '12px', whiteSpace: 'pre-line' }}>
                {getBody(p)}
              </p>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{
                    ...chipBase,
                    background: myVote?.vote === 'support' ? '#dcfce7' : 'transparent',
                    color: myVote?.vote === 'support' ? '#166534' : 'var(--txm)',
                    borderColor: myVote?.vote === 'support' ? 'transparent' : 'var(--br)',
                    fontWeight: myVote?.vote === 'support' ? 500 : 400,
                ...
                <button
                  ...
                  style={{
                    ...chipBase,
                    background: myVote?.vote === 'against' ? '#fee2e2' : 'transparent',
                    color: myVote?.vote === 'against' ? '#991b1b' : 'var(--txm)',
                    borderColor: myVote?.vote === 'against' ? 'transparent' : 'var(--br)',
                    fontWeight: myVote?.vote === 'against' ? 500 : 400,
                    display: 'flex', alignItems: 'center', gap: '4px',
                  }}
                >
                  👎 Against ({p.against})
