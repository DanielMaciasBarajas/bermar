...
-- Run this in Supabase SQL editor (eu-west-1 / Ireland)
-- ============================================================
...
-- ============================================================
-- COMMUNITIES (multi-tenant config)
-- ============================================================
...
-- Generate all Beramar apartments
do $$
declare
  community_id uuid;
  floor_num integer;
  ...
  upper_apt text;
begin
  select id into community_id from communities where slug = 'beramar';
  for floor_num in 1..9 loop
    foreach door_letter in array doors loop
      apt := floor_num::text || door_letter;
      is_dup := (floor_num = 8);
      upper_apt := case when is_dup then '9' || door_letter else null end;
      insert into apartments (community_id, apt_number, floor, door, is_duplex, duplex_upper_number)
      values (community_id, apt, floor_num, door_letter, is_dup, upper_apt);
      -- Also insert 9th floor duplex partners
      if floor_num = 8 then
        insert into apartments (community_id, apt_number, floor, door, is_duplex, duplex_upper_number)
        values (community_id, '9' || door_letter, 9, door_letter, true, apt)
...
-- ============================================================
-- PROFILES (one per registered apartment)
-- ============================================================
...
-- ============================================================
-- OCCUPANTS (people living in each apartment)
-- ============================================================
...
-- ============================================================
-- PREMISES (bookable spaces per community)
-- ============================================================
...
from communities c
cross join (values
  ('Tennis court', '{"CA":"Pista de tennis","ES":"Pista de tenis","FR":"Court de tennis"}', 'court', 'slots', 90, true, 3, 'tennis', 1),
  ('Padel court', '{"CA":"Pista de pàdel","ES":"Pista de pádel","FR":"Court de padel"}', 'court', 'slots', 90, true, 3, 'padel', 2),
  ('Football 5', '{"CA":"Futbol 5","ES":"Fútbol 5","FR":"Foot à 5"}', 'court', 'slots', 90, true, 9, 'football', 3),
  ('Basketball', '{"CA":"Bàsquet","ES":"Baloncesto"}', 'court', 'slots', 90, true, 9, 'basketball', 4),
  ('Ping pong', '{"CA":"Ping-pong","ES":"Ping-pong"}', 'court', 'slots', 90, true, 3, 'ping-pong', 5),
  ('BBQ area', '{"CA":"Zona barbacoa","ES":"Zona barbacoa","FR":"Zone barbecue"}', 'social', 'halfday', 0, false, null, 'bbq', 6),
  ('Party room', '{"CA":"Sala de festes","ES":"Sala de fiestas","FR":"Salle des fêtes"}', 'social', 'halfday', 0, false, null, 'party', 7),
  ('Chess / Escacs', '{"CA":"Escacs","ES":"Ajedrez","FR":"Échecs"}', 'challenge', 'challenge', 0, false, 1, 'chess', 8)
...
-- ============================================================
-- BOOKING PARTICIPANTS (invite joiners)
-- ============================================================
...
-- ============================================================
-- PROPOSAL PERSONAL FLAGS (per user)
-- ============================================================
...
-- ============================================================
-- COMMENTS (proposals, events, marketplace)
-- ============================================================
...
-- ============================================================
-- WARNINGS / ANNOUNCEMENTS / CONVOCATORIA
-- ============================================================
...
-- ============================================================
-- COMMUNITY VOICE (auto-generated posts)
-- ============================================================
...
-- Insert Beramar emergency contacts
...
-- Helper function: get current user's community_id
...
-- Communities: residents see their own, SA sees all
create policy "Residents see own community" on communities
  for select using (id = get_my_community_id() or get_my_role() = 'sa');

-- Apartments: residents see apartments in their community
create policy "Residents see community apartments" on apartments
  for select using (community_id = get_my_community_id());

-- Profiles: residents see approved profiles in their community
create policy "Residents see community profiles" on profiles
...
-- Occupants
create policy "Residents see occupants in community" on occupants
  for select using (
    profile_id in (select id from profiles where community_id = get_my_community_id() and approved = true)
  );
create policy "Own occupants" on occupants
  for all using (profile_id = auth.uid());

-- Interests
create policy "Residents see interests" on interests
  for select using (
    profile_id in (select id from profiles where community_id = get_my_community_id() and approved = true)
  );
...
-- Premises: all approved residents see community premises
create policy "Residents see premises" on premises
  for select using (community_id = get_my_community_id());

-- Bookings
create policy "Residents see community bookings" on bookings
  for select using (community_id = get_my_community_id());
...
create policy "See booking participants" on booking_participants
  for select using (
    booking_id in (select id from bookings where community_id = get_my_community_id())
  );
...
-- Proposals
create policy "Residents see proposals" on proposals
  for select using (community_id = get_my_community_id());
...
-- Comments
create policy "Residents see comments" on comments
  for select using (community_id = get_my_community_id());
create policy "Create comment" on comments
  for insert with check (profile_id = auth.uid() and community_id = get_my_community_id());

-- Marketplace
create policy "Residents see marketplace" on marketplace_listings
...
-- Documents
create policy "Residents see documents" on documents
  for select using (community_id = get_my_community_id());
create policy "Admin uploads documents" on documents
  for insert with check (get_my_role() in ('admin','sa'));
create policy "Document files" on document_files
  for select using (
    document_id in (select id from documents where community_id = get_my_community_id())
  );
create policy "Admin uploads files" on document_files
  for insert with check (get_my_role() in ('admin','sa'));

-- Projects
create policy "Residents see projects" on projects
  for select using (community_id = get_my_community_id());
create policy "Admin manages projects" on projects
  for all using (get_my_role() in ('admin','sa'));
create policy "See project updates" on project_updates
  for select using (
    project_id in (select id from projects where community_id = get_my_community_id())
  );
...
-- Announcements
create policy "Residents see announcements" on admin_announcements
...
-- Community voice
create policy "Residents see voice" on community_voice
  for select using (community_id = get_my_community_id());
...
-- Emergency contacts
create policy "Residents see emergency contacts" on emergency_contacts
  for select using (community_id = get_my_community_id());
create policy "Admin manages emergency contacts" on emergency_contacts
  for all using (get_my_role() in ('admin','sa'));

-- Activity log
create policy "Admin sees activity log" on activity_log
  for select using (get_my_role() in ('admin','sa') and community_id = get_my_community_id());
create policy "SA sees all logs" on activity_log
  for select using (get_my_role() = 'sa');
...
create trigger trg_marketplace_updated_at before update on marketplace_listings
  for each row execute function update_updated_at();
create trigger trg_maintenance_updated_at before update on maintenance_tickets
  for each row execute function update_updated_at();

-- Auto-create profile on auth signup (called via Edge Function)
-- Auto-log activity
...
begin
  insert into activity_log (community_id, actor_id, action, entity_type, entity_id, metadata)
  values (p_community_id, auth.uid(), p_action, p_entity_type, p_entity_id, p_metadata);
end;
$$;
...
create index idx_bookings_community_date on bookings(community_id, date);
create index idx_bookings_premise on bookings(premise_id, date);
create index idx_proposals_community on proposals(community_id, status);
create index idx_marketplace_community on marketplace_listings(community_id, status);
create index idx_notifications_profile on notifications(profile_id, read);
create index idx_maintenance_community on maintenance_tickets(community_id, status);
create index idx_activity_community on activity_log(community_id, created_at desc);
